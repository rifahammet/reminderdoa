...
class ApprovalStoreSampahPage extends StatefulWidget {
  final bool? isViewOnly;
  const ApprovalStoreSampahPage({Key? key, this.isViewOnly}) : super(key: key);
  @override
  _ApprovalStoreSampahPageState createState() =>
      _ApprovalStoreSampahPageState();
}

class _ApprovalStoreSampahPageState extends State<ApprovalStoreSampahPage> {
    ...
    final dataPropinsi = auth.getGlobalParam(
        ...
        like: txtFilter.text.toString().trim() != ""
            ? {
                ...
                "approved": 0,
                "permanent_deleted": 0,
                "user-profile_id": Prefs.getInt("userId"),
          ...
          "data": [
            {
              ...
              "total_approved": double.parse(data["total"]),
              "dt_approved":
                  Fungsi().fmtDateTimeYearNow(),
              "first_approved": 1,
              "permanent_deleted":
                  Fungsi().StringToBool(data["deleted"]) ? 1 : 0
            }
          ]
        }
      },
      {
        "approvalstoresampah": {
          ...
          "data": [
            {
              "total_approved": double.parse(data["total"]),
              "store_sampah_id_header": data["id"],
              "approved_user_profile_id": Prefs.getInt("userId"),
            }
          ]
        }
          ...
          Expanded(
            child: Column(
              ...
              children: <Widget>[
                ...
                Text(
                  formatDate(
                      DateTime.parse(newData?[index]['tanggal_transaksi']),
                      [dd, '-', mm, '-', yyyy]),
                  style: TextStyle(
                      color: Colors.grey[700],
                      fontSize: 12,
