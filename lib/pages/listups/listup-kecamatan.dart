...
class ListUpKecamatanDialog extends StatefulWidget {
  final String? kota_kode;
  const ListUpKecamatanDialog({Key? key, this.kota_kode}) : super(key: key);
  @override
  _ListUpKecamatanDialogState createState() => _ListUpKecamatanDialogState();
}

class _ListUpKecamatanDialogState extends State<ListUpKecamatanDialog> {
  ...
  Future getDataKebun(
    ...
    final dataPropinsi = auth.getGlobalParam(
        namaApi: "kecamatan", like: txtFilter.text.toString().trim()!=""? { "active": "1","kota_kode": widget.kota_kode,"concat_field": txtFilter.text}:{ "active": "1","kota_kode": widget.kota_kode}, startFrom: (iPos-1) * int.parse(_selectedType.toString()) , limit: _selectedType);
