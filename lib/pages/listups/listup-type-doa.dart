...
class ListUpTypeDoaPage extends StatefulWidget {
  const ListUpTypeDoaPage({Key? key}) : super(key: key);
  @override
  _ListUpTypeDoaPageState createState() => _ListUpTypeDoaPageState();
}

class _ListUpTypeDoaPageState extends State<ListUpTypeDoaPage> {
  ...
  Future getDataKebun(
    ...
    final dataPropinsi = auth.getGlobalParam(
        namaApi: "typesdoa", like: txtFilter.text.toString().trim()!=""? {"concat_field": txtFilter.text}:{}, startFrom: (iPos-1) * int.parse(_selectedType.toString()) , limit: _selectedType);
